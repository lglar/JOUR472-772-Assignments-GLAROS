...
evans_response <- axolotr::ask(
  prompt = paste("Given the following text, extract information into a CSV file with the following structure about every film and tv show mentioned in the article: title, year, role, director, co-stars, and award. NO YAPPING. If no information is mentioned, put NA. Please format it in typical chart format. If there are multiple variables in a section, seperate them with a comma", text),
  model = "llama-3.1-8b-instant"
)
...
**Answer:**
Not all the information came from the text, such as director names. In addition, the name of the role she played in California Suite is wrong.
------------------------------------------------------------------------
...
#Add code below to talk to Groq and display its response
evans_response <- axolotr::ask(
  prompt = paste("Given the following code, explain what the code does", r_code_example),
  model = "llama-3.1-8b-instant"
)
...
**Answer:**
It is accurate, but it appears that its designation of the capitalization of Climate as being case insensitive is incorrect.
------------------------------------------------------------------------
...
your_r_code_lab = "maryland_expenses_by_location <- maryland_expenses |> 
  mutate(
...
#Add code below to talk to Groq and display its response
evans_response <- axolotr::ask(
  prompt = paste("Given the following code, explain if the code is correct given the question", your_r_code_lab,lab_question),
  model = "llama-3.1-8b-instant"
)
...
**Answer:**
The answer makes sense because it accurately described what my code does, but the only part that doesn't make sense is the part where it said my filter didn't work, because it just repeated the exact same code I used in the correction.
------------------------------------------------------------------------
...
idea_response <- axolotr::ask(
  prompt = "I have a CSV file of data about each collision and details of all traffic collisions occurring on county and local roadways within Montgomery County, including the type of incident, building location and time. Suggest some story ideas using this data",
  model = "llama-3.1-8b-instant"
)
...
**Answer:**
I think the ideas seem pretty good, especially the suggestion about collision hot spots, because that could be an interesting story in terms of roadway design failures.
------------------------------------------------------------------------
...
**Answer:** It hallucinated quotes, and there isnt even an intersection between the two roads. It also made up a repeated traffic problem.Also I'm pretty sure Knox Road doesnt go North. 
