...
acccounty_911_calls_combined <- bind_rows(list(carroll_911, cecil_911, allegany_911))
  View(acccounty_911_calls_combined)
```
...
bmore_911 <- read_csv("data/baltimore_911.csv") |> clean_names()
maryland_dem_data <- read_csv("data/maryland_zcta (2).csv")|> clean_names()

bmore_911_y_dem <- bmore_911 |> left_join(maryland_dem_data, join_by(zip_code==zcta5n))
...
A2. There are usually around 30-50 calls per month in each county, with the higher calls often being in August. Allegany is an outlier because it has five of the top six spots but had the least amount of total calls.

314 GROVE NECK RD EARLEVILLE, MD in Cecil County had the most calls, with 15. The address is a recovery center.
...
loc_y_co_calls <- acccounty_911_calls_combined |>
...
The zip codes are mostly concentrated in the center-west of the city. The zip code with the highest rate of calls per 1000 is 21217 with a rate of around 20.9, and one of the neighborhoods is Penn North which has 37111 people and 81.5% are under 18 and 70% rent.
...
rt_per_1000_bmore <- bmore_911_y_dem |>
filter(pct_under_18 >= 75)|>
group_by(zip_code, population) |>
summarize(call_count=n())|>
mutate(rt_per_1000 = (call_count / population)* 1000 )|>
arrange(desc(rt_per_1000))

rt_per_neigh <- bmore_911_y_dem |>
filter(zip_code == 21217) |>
filter(pct_under_18 >= 75)|>
group_by(zip_code, population, neighborhood, pct_under_18, pct_rent) |>
summarize(call_count=n())|>
mutate(rt_per_1000 = (call_count / population)* 1000 )|>
arrange(desc(rt_per_1000))
...
What is the neighborhood with the most overdoses where the population is over 50% homeowners.
