...
A2:The 2021 FY had the largest total amount.This stands out to  me because it was a COVID year, which could explain more expenditures.
...
A3.
PLANO-CONDON, LLC and PLANO-CONDON LLC could be combined into one, A MORTON THOMAS & ASSOCIATES INC and A MORTON THOMAS & ASSOC INC
could be combined into one, and AEROSOL MONITORING & ANALYSIS INC
and AERSOL MONITORING & ANALYSIS INC could be combined into one. In order to clean the data to make nearly identical vendors identical you would need to standarize the names to the companies correct name by finding each name variation and mutate case_when to the official name.
...
A4:
Funding for stem cell research has gone down over the years, but 2023 is an outlier.

```{r}
sc_research_grants <- cleaned_md_grants_loans |>
filter(
str_detect(description, "STEM CELL RESEARCH GRANT") | 
str_detect(description, "Stem Cell Research Grant")
) 

stem_money <- sc_research_grants |>
group_by(fiscal_year) |>
summarise(total_spent=sum(amount)) |>
arrange(desc(total_spent))
...
A5:I had 18 grantees. I would need to clean the data using case_when to standardize all the UMD names. I thought the "GripBoost" company seemed interesting because I had no idea what it meant, turns out they make football gloves. I also find it interesting that there are some companies in the zip code that don't have UMD in the name.

```{r}
umd_grantees <- cleaned_md_grants_loans |>
filter(
  str_detect(zip_code, "20742")
)

umd_money <- umd_grantees |>
group_by(grantee) |>
summarise(total_spent=sum(amount)) |>
arrange(desc(total_spent))
