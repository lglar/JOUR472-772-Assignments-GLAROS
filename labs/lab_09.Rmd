...
**A1.**
https://datawrapper.dwcdn.net/iMbj5/1/
I decided to do a range plot because it would show the decent differences between what the two recieved in their elections.
```{r}
md_gov_county <- read_csv("data/md_gov_county.csv")|> clean_names()
md_gov_county18 <- read_csv("data/md_gov_county_18.csv")|> clean_names() 
md_gov_county_last8 <- md_gov_county |> full_join(md_gov_county18, join_by(county==county, geoid == geoid)) 

md_gov_county_last8 <- md_gov_county_last8 |>
mutate(total_22 = cox + moore +lashar + wallace + harding + write_ins.x, pct_cox = (cox / (total_22)* 100)) |>
mutate(total_18 = hogan + jealous + quinn + schlakman + write_ins.y, pct_hogan = (hogan/ (total_18))*100 
) 

write_csv(md_gov_county_last8, "data/md_gov_county_last8_expanded.csv")
...
**A2.**
https://datawrapper.dwcdn.net/WAbz4/1/
I think the story is that mail-in voting spiked during the pandemic.
```{r}
...
md_votes_by_type <- md_votes_by_type|>
  ...
  mutate(pct_elecday = (election_day/total)*100) |>
  mutate(pct_mail = (mail/total)*100)
write_csv(md_votes_by_type, "data/md_votes_by_type_expanded.csv")
```
...
**A3.**
https://datawrapper.dwcdn.net/oHAim/2/
I think the story is that zip codes near the center of the county and in the southeast have higher rates of 911 calls, and I would like to know more about the income level of these areas.

```{r}
monty_overdoses_22 <- read_csv("data/montgomery_2022_overdoses.csv")|> clean_names()
monty_overdoses_22_ext <- monty_overdoses_22 |>
group_by(zip) |>
summarize(total_calls = n()) |>
mutate(pct = (total_calls/sum(total_calls))*100)
write_csv(monty_overdoses_22_ext, "data/monty_overdoses_22_ext.csv")
```
