...
The depth is 643.060 and the magnitude was 4.20---------------------------------------------------------------------
...
12 earthquakes fit this category. ------------------------------------------------------------------------
...
There are 8 earthquakes that fit the criteria.------------------------------------------------------------------------
...
**A7:**
You should look at the place column and the number is 2,355
------------------------------------------------------------------------
...
**A8:**
The two most common tremors are earthquakes and quarry blasts.
------------------------------------------------------------------------
...
-------The mean deoth is 19.55 and it seems to not be negative, which is odd. It is also far larger than the other types.-----------------------------------------------------------------
...
```{r}
earthquakes |>
  mutate(serious_tremors = mag > 3)
```

**A11:**
You mutate the column
------------------------------------------------------------------------
...
**A12**:
Hour 16 has the most and hour 0 has the least.
------------------------------------------------------------------------

#### EXTRA Q What other questions are you interested in and how would you ask them in R?
How many earthquakes happened in each area?
```{r}
earthquakes |> 
group_by(place) |>
summarize(type_count=n())|> 
arrange(desc(type_count))
```
How many times has an earthquake been recorded in Russia in the data set? 226 times
```{r}
earthquakes |> 
filter(str_detect(place, "Russia"))

```
What was the magnitude of the largest earthquake recorded in Russia? 6.1
```{r}
earthquakes |> 
filter(str_detect(place, "Russia"))|> 
arrange(desc(mag))
