...
**Task** Run the following code. Describe the output in the space below. **Answer**
It gives the number of population and forclosure notices.
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
It downloaded the geometry of each county, including coodinates to a column.
```{r}
md_county_population <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              year = 2021,
...
**Task** Run the following code. Describe the output in the space below. **Answer**
It lists all the counties but twice in two columns.
```{r}
md_county_population
...
**Task** Run the following code. Describe the output in the space below. **Answer**
It removed the word Maryland from the names
...
**Task** Run the following code. Describe the output in the space below. **Answer**
It joined all the data together without duplicates.
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
It calculated the rate of foreclosures for each county.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The data was not working initially so I changed the dataset it was pulling from and changed name to counties. It created a gradient of rates.
```{r}

county_centroids <- st_centroid(md_pop_with_foreclosures)
...
The differences are even more specific. The rates are higher in southern and some parts of eastern Maryland,.
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
