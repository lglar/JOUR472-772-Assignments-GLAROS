...
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=month, weight=total_calls, fill = 'red'))
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer The titles are now more professional and aesthetically pleasing, and are more descriptive.**
...
Each date now has a number of calls next to it.
...
I see a line chart depicting the number of calls by date.
...
More specific dates are now shown, but it's unreadable.
...
The dates are now slanted, which makes it slightly more readable.
