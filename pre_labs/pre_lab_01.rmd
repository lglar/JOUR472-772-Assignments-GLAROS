...
```{r}
install.packages('tidyverse', 'markdown')
library(tidyverse)
```
...
**Answer** It is ordering the amount of classes by term \### Task 10: Adding arrange to sort the data
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer It changed the class count to ascending, which meant it is no longer ordered by year**
...
**Answer It grouped by both term and department, the group by (term, department) changed the code**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer The number of seats has been included which now sorts the sheet in ascending order of seats.**
...
1.  Which instructor teaches the most classes each term and how many people take those classes?

2.  Which classes are taught year-round and which classes are only taught in certain terms?

3.  Which department has the highest concentration of classes with more than 100 seats?
